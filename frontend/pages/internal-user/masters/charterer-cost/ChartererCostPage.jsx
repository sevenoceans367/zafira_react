...
} from '../../../../services/chartererCosts.js';
import MastersHeaderActions from '../MastersHeaderActions.jsx';
import { filterMasterRows } from '../filterMasterRows.js';
import styles from './ChartererCostPage.module.css';

const FLASH_MESSAGES = {
  0: { type: 'success', text: 'Congratulations! Charterer Cost added/updated successfully.' },
  1: { type: 'error', text: 'Sorry! there was an error while adding/updating Charterer Cost.' },
...
export default function ChartererCostPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchChartererCosts();
      setRows(data.records ?? []);
    } catch (err) {
      setError(err.message || 'Failed to load charterer cost list.');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (id) => {
    ...
    setError('');
    try {
      const record = await fetchChartererCost(id);
      setEditId(id);
      setForm({ name: record.name || '' });
      setView('form');
    } catch (err) {
      setError(err.message || 'Failed to load charterer cost.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (row, event) => {
    ...
    try {
      const result = await updateChartererCostStatus(row.id, row.status);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = editId
        ? await updateChartererCost(editId, form)
        : await createChartererCost(form);
      ...
      setError(err.message || 'Failed to save charterer cost.');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className={`zafira-page ${styles.page}`}>
        ...
        <h3 className={styles.title}>
          {editId ? 'Edit Charterer Cost' : 'Add Charterer Cost'}
        </h3>
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      <MastersHeaderActions
        ...
        searchPlaceholder="Search charterer costs"
        onAdd={openAdd}
      />

      {loading ? <LoadingOverlay active label="Loading charterer costs…" /> : null}

      <h3 className={styles.title}>Charterer Cost List</h3>
            ...
            {filteredRows.length === 0 && !loading ? (
              <tr>
                <td className={styles.emptyCell} colSpan={4}>
                  No charterer cost records found.
                </td>
              </tr>
            ) : null}
