...
} from '../../../../services/accountingGroups.js';
import MastersHeaderActions from '../MastersHeaderActions.jsx';
import { filterMasterRows } from '../filterMasterRows.js';
import styles from './AccountingGroupPage.module.css';

const FLASH_MESSAGES = {
  0: { type: 'success', text: 'Congratulations! Accounting Group added/updated successfully.' },
  1: { type: 'error', text: 'Sorry! there was an error while adding/updating Accounting Group.' },
...
export default function AccountingGroupPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchAccountingGroups();
      setRows(data.records ?? []);
    } catch (err) {
      setError(err.message || 'Failed to load Accounting Group list.');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (id) => {
    ...
    setError('');
    try {
      const record = await fetchAccountingGroup(id);
      setEditId(id);
      setForm({ name: record.name || '' });
      setView('form');
    } catch (err) {
      setError(err.message || 'Failed to load Accounting Group.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (row, event) => {
    ...
    try {
      const result = await updateAccountingGroupStatus(row.id, row.status);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = editId
        ? await updateAccountingGroup(editId, form)
        : await createAccountingGroup(form);
      ...
      setError(err.message || 'Failed to save Accounting Group.');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className={`zafira-page ${styles.page}`}>
        ...
        <h3 className={styles.title}>
          {editId ? 'Edit Accounting Group' : 'Add Accounting Group'}
        </h3>
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      <MastersHeaderActions
        ...
        searchPlaceholder="Search accounting groups"
        onAdd={openAdd}
      />

      {loading ? <LoadingOverlay active label="Loading accounting groups…" /> : null}

      <h3 className={styles.title}>Accounting Group List</h3>
            ...
            {filteredRows.length === 0 && !loading ? (
              <tr>
                <td className={styles.emptyCell} colSpan={4}>
                  No Accounting Group records found.
                </td>
              </tr>
            ) : null}
