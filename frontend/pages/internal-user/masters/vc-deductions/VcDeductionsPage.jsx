...
} from '../../../../services/vcDeductions.js';
import MastersHeaderActions from '../MastersHeaderActions.jsx';
import { filterMasterRows } from '../filterMasterRows.js';
import styles from './VcDeductionsPage.module.css';

const FLASH_MESSAGES = {
  0: { type: 'success', text: 'Congratulations! VC Deduction added/updated successfully.' },
  1: { type: 'error', text: 'Sorry! there was an error while adding/updating VC Deduction.' },
...
export default function VcDeductionsPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchVcDeductions();
      setRows(data.records ?? []);
    } catch (err) {
      setError(err.message || 'Failed to load VC Deduction list.');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (id) => {
    ...
    setError('');
    try {
      const record = await fetchVcDeduction(id);
      setEditId(id);
      setForm({ name: record.name || '' });
      setView('form');
    } catch (err) {
      setError(err.message || 'Failed to load VC Deduction.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (row, event) => {
    ...
    try {
      const result = await updateVcDeductionStatus(row.id, row.status);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = editId
        ? await updateVcDeduction(editId, form)
        : await createVcDeduction(form);
      ...
      setError(err.message || 'Failed to save VC Deduction.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      <MastersHeaderActions
        ...
        searchPlaceholder="Search VC deductions"
        onAdd={openAdd}
      />
