...
export default function OpsVcInOpsGlancePage() {
  ...
  const handleAddSheetClick = async (row) => {
    // PHP: always show "A"; open modal only when last VF is Submit to Close
        ...
        <table className={styles.table}>
          <thead>
            <tr>
              ...
              <th className={styles.alertsCell}>Alerts</th>
              <th>Operator</th>
            ...
            {!rows.length && !loading ? (
              ...
              <tr key={row.comId}>
                <td className={styles.actionsCell}>
                  <Link to={appPath(`/internal-user/sopf/viewestimate?id=${row.fcaId}&rttype=1`)}>FVF</Link>
                ...
                <td>
                  ...
                  <div>
                    {row.vesselImoNo ? (
                      <Link
                        to={appPath(`/internal-user/vc/ops/voyage-report?vesselimono=${encodeURIComponent(row.vesselImoNo)}&comid=${encodeURIComponent(row.comId)}&page=1&type=VC&selYear=${encodeURIComponent(year)}`)}
                      >
                        Voyage Report
                      </Link>
                    ) : (
                ...
                <td className={styles.actionsCell}>
                  {(row.costSheets || []).map((sheet) => (
                    <div key={sheet.id}>
                      <Link
                        to={appPath(`/internal-user/vc/ops/cost-sheet?comid=${encodeURIComponent(row.comId)}&cost_sheet_id=${encodeURIComponent(sheet.id)}&page=1`)}
                      >
                        {sheet.name}
                      </Link>
                    </div>
                ...
                <td className={`${styles.actionsCell} ${styles.alertsCell}`}>
                  ...
                  {row.paymentNotReceived ? <div className={styles.alertText}>Payment not Received</div> : null}
                  {row.paymentNotPaid ? <div className={styles.alertText}>Payment not Paid</div> : null}
                </td>
                <td>
                  {canEditOperator ? (
