...
} from '../../../../services/tcDeductions.js';
import MastersHeaderActions from '../MastersHeaderActions.jsx';
import { filterMasterRows } from '../filterMasterRows.js';
import styles from './TcDeductionsPage.module.css';

const FLASH_MESSAGES = {
  0: { type: 'success', text: 'Congratulations! TC Deduction added/updated successfully.' },
  1: { type: 'error', text: 'Sorry! there was an error while adding/updating TC Deduction.' },
...
export default function TcDeductionsPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchTcDeductions();
      setRows(data.records ?? []);
    } catch (err) {
      setError(err.message || 'Failed to load TC Deduction list.');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (id) => {
    ...
    setError('');
    try {
      const record = await fetchTcDeduction(id);
      setEditId(id);
      setForm({ name: record.name || '' });
      setView('form');
    } catch (err) {
      setError(err.message || 'Failed to load TC Deduction.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (row, event) => {
    ...
    try {
      const result = await updateTcDeductionStatus(row.id, row.status);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = editId
        ? await updateTcDeduction(editId, form)
        : await createTcDeduction(form);
      ...
      setError(err.message || 'Failed to save TC Deduction.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      <MastersHeaderActions
        ...
        searchPlaceholder="Search TC deductions"
        onAdd={openAdd}
      />
