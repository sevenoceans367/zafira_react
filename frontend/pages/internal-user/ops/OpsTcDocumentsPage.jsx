...
export default function OpsTcDocumentsPage() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const result = await fetchOpsTcDocuments(comId);
      setData(result);
    } catch (err) {
      setData(null);
  ...
  const handleSubmit = async (event) => {
    ...
    setError('');
    try {
      await createOpsTcDocument(comId, { fileName: fileName.trim() }, files);
      setFileName('');
      setFiles([]);
      await load();
    } catch (err) {
  ...
  const handleDelete = async (doc) => {
    ...
    setError('');
    try {
      await deleteOpsTcDocument(comId, doc.storedFiles);
      await load();
    } catch (err) {
    ...
    <div className={`zafira-page ${styles.page}`}>
      ...
      <form className={styles.letterPanel} onSubmit={handleSubmit}>
        <div className={styles.formGrid}>
          <Field id="ops-tc-doc-file-name" label="File Name">
            <TextInput
              id="ops-tc-doc-file-name"
              value={fileName}
          ...
          <Field id="ops-tc-doc-files" label="Attachment">
            <input
              id="ops-tc-doc-files"
              type="file"
              multiple
