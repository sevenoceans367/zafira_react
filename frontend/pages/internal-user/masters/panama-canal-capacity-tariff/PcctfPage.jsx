...
import {
  ...
  updatePcctf,
  updatePcctfStatus,
} from '../../../../services/pcctf.js';
import MastersHeaderActions from '../MastersHeaderActions.jsx';
import { filterMasterRows } from '../filterMasterRows.js';
import styles from './PcctfPage.module.css';

const FLASH_MESSAGES = {
  0: {
    type: 'success',
    text: 'Congratulations! Panama Canal Capacity Tariff Fee added/updated successfully.',
  },
  1: {
    type: 'error',
    text: 'Sorry! there was an error while adding/updating Panama Canal Capacity Tariff Fee.',
  },
...
export default function PcctfPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchPcctfList();
      setRows(data.records ?? []);
    } catch (err) {
      setError(err.message || 'Failed to load Panama Canal Capacity Tariff Fee list.');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadLookups = useCallback(async () => {
    try {
      const data = await fetchPcctfLookups();
      setLookups({
  ...
  const openEdit = async (id) => {
    ...
    setError('');
    try {
      const record = await fetchPcctf(id);
      setEditId(id);
      setForm({
      ...
      setError(err.message || 'Failed to load Panama Canal Capacity Tariff Fee.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (row, event) => {
    ...
    try {
      const result = await updatePcctfStatus(row.id, row.status);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = editId
        ? await updatePcctf(editId, form)
        : await createPcctf(form);
      ...
      setError(err.message || 'Failed to save Panama Canal Capacity Tariff Fee.');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className={`zafira-page ${styles.page}`}>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <label className={styles.field}>
            <span className={styles.label}>Per PC UMS Ton / Rate per DWT (USD)</span>
            <input
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      <MastersHeaderActions
        ...
        searchPlaceholder="Search Panama Canal Capacity Tariff Fee"
        onAdd={openAdd}
      />
        ...
        <table className={`zafira-data-table ${styles.table}`}>
          <thead>
            <tr>
              ...
              <th>Per PC UMS Ton / Rate per DWT (USD)</th>
              <th>Status</th>
              <th>Details</th>
            </tr>
          </thead>
            ...
            {filteredRows.length === 0 && !loading ? (
              <tr>
                <td className={styles.emptyCell} colSpan={6}>
                  No Panama Canal Capacity Tariff Fee records found.
                </td>
              </tr>
            ) : null}
