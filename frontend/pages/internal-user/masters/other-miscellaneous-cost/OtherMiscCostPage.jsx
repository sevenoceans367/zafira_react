...
} from '../../../../services/otherMiscCosts.js';
import MastersHeaderActions from '../MastersHeaderActions.jsx';
import { filterMasterRows } from '../filterMasterRows.js';
import styles from './OtherMiscCostPage.module.css';

const FLASH_MESSAGES = {
  0: { type: 'success', text: 'Congratulations! Other Miscellaneous Cost added/updated successfully.' },
  1: { type: 'error', text: 'Sorry! there was an error while adding/updating Other Miscellaneous Cost.' },
...
export default function OtherMiscCostPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchOtherMiscCosts();
      setRows(data.records ?? []);
    } catch (err) {
      setError(err.message || 'Failed to load other miscellaneous cost list.');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (id) => {
    ...
    setError('');
    try {
      const record = await fetchOtherMiscCost(id);
      setEditId(id);
      setForm({
        name: record.name || '',
      ...
      setError(err.message || 'Failed to load other miscellaneous cost.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (row, event) => {
    ...
    try {
      const result = await updateOtherMiscCostStatus(row.id, row.status);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = editId
        ? await updateOtherMiscCost(editId, form)
        : await createOtherMiscCost(form);
      ...
      setError(err.message || 'Failed to save other miscellaneous cost.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      <MastersHeaderActions
        ...
        searchPlaceholder="Search other miscellaneous costs"
        onAdd={openAdd}
      />
            ...
            {filteredRows.length === 0 && !loading ? (
              <tr>
                <td className={styles.emptyCell} colSpan={4}>
                  No other miscellaneous cost records found.
                </td>
              </tr>
            ) : null}
