...
export async function downloadSensitivityAnalysisPdf(payload) {
  const response = await fetch(`${BASE}/sensitivity_analysis/pdf`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
  ...
  if (!response.ok) {
    const data = await response.json().catch(() => ({}));
    throw new Error(data.message || 'Failed to generate sensitivity analysis PDF.');
  }
