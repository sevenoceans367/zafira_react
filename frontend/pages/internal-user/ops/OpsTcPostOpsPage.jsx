...
export default function OpsTcPostOpsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load Vessels in Post Ops TC.');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      <div className={`zafira-page ${styles.page}`}>
        {loading || savingSheet ? (
          <LoadingOverlay active label={savingSheet ? 'Creating sheet…' : 'Loading Vessels in Post Ops TC…'} />
        ) : null}
              ...
              {!rows.length && !loading ? (
                ...
                <tr key={row.comId}>
                  ...
                  <td>
                    ...
                    <br />
                    <Link
                      to={appPath(`/internal-user/vc/ops-tc/fixture-note?comid=${encodeURIComponent(row.comId)}&page=${PAGE_CONTEXT}`)}
                    >
                      Fixture Note
                    </Link>
                  </td>
