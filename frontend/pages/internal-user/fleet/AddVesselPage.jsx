...
import { createVesselPrimary, fetchVesselPrimaryLookups } from '../../../services/fleet.js';
...
export default function AddVesselPage() {
  ...
  const loadLookups = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchVesselPrimaryLookups();
      setLookups(data.lookups);
    } catch (err) {
      setError(err.message || 'Failed to load vessel form.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await createVesselPrimary(payload);
      navigate(`${fleetPath}?msg=0&selBType=${form.businessTypeId}`, { replace: true });
    } catch (err) {
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      {(loading || saving) ? (
        <LoadingOverlay active label={saving ? 'Saving vessel…' : 'Loading vessel form…'} />
      ) : null}
      ...
      <section className={styles.panel}>
        <div className={styles.panelHeader}>ADD NEW VESSEL</div>
        <form className={styles.panelBody} onSubmit={handleSubmit}>
          <div className={styles.formGrid}>
            ...
            <FormField
              ...
              label="IMO number"
              value={form.imoNo}
              onChange={(value) => updateField('imoNo', value)}
              required
            />
            <FormField
              id="vesselName"
              ...
              onChange={(value) => updateField('vesselName', value)}
              required
            />
            <FormField
              id="vesselCode"
              ...
              onChange={(value) => updateField('vesselCode', value)}
              required
            />
            <FormField
              id="yearBuilt"
            ...
            <FormField
              ...
              label="Summer DWT (MT)"
              value={form.dwt}
              onChange={(value) => updateField('dwt', value)}
              required
            />
          ...
          <div className={styles.footer}>
            <Button variant="primary" label="Submit" type="submit" disabled={saving || loading} />
          </div>
        </form>
      </section>
    </div>
