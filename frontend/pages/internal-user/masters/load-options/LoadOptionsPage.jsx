...
} from '../../../../services/loadOptions.js';
import MastersHeaderActions from '../MastersHeaderActions.jsx';
import { filterMasterRows } from '../filterMasterRows.js';
import styles from './LoadOptionsPage.module.css';

const FLASH_MESSAGES = {
  0: {
    type: 'success',
    text: 'Congratulations! Load Options List added/updated successfully.',
  },
  1: {
    type: 'error',
    text: 'Sorry! there was an error while adding/updating Load Options.',
  },
...
export default function LoadOptionsPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchLoadOptions();
      setRows(data.records ?? []);
    } catch (err) {
      setError(err.message || 'Failed to load Load Options list.');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (id) => {
    ...
    setError('');
    try {
      const record = await fetchLoadOption(id);
      setEditId(id);
      setForm({ name: record.name || '' });
      setView('form');
    } catch (err) {
      setError(err.message || 'Failed to load Load Option.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (row, event) => {
    ...
    try {
      const result = await updateLoadOptionStatus(row.id, row.status);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = editId
        ? await updateLoadOption(editId, form)
        : await createLoadOption(form);
      ...
      setError(err.message || 'Failed to save Load Option.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className={`zafira-page ${styles.page}`}>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          <label className={styles.field}>
            <span className={styles.label}>Load Options</span>
            <input
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      <MastersHeaderActions
        ...
        searchPlaceholder="Search Load Options"
        onAdd={openAdd}
      />
