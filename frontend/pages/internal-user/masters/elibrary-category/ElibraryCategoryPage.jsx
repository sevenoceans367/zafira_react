...
import {
  ...
  fetchElibraryCategories,
  fetchElibraryCategory,
  updateElibraryCategory,
  updateElibraryCategoryStatus,
} from '../../../../services/elibraryCategories.js';
import MastersHeaderActions from '../MastersHeaderActions.jsx';
import { filterMasterRows } from '../filterMasterRows.js';
import styles from './ElibraryCategoryPage.module.css';

const FLASH_MESSAGES = {
  0: {
    type: 'success',
    text: 'Congratulations! E-Library Category Master added/updated successfully.',
  },
  1: {
    type: 'error',
    text: 'Sorry! there was an error while adding/updating E-Library Category Master.',
  },
...
export default function ElibraryCategoryPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchElibraryCategories();
      setRows(data.records ?? []);
    } catch (err) {
      setError(err.message || 'Failed to load E-Library category list.');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (id) => {
    ...
    setError('');
    try {
      const record = await fetchElibraryCategory(id);
      setEditId(id);
      setForm({ name: record.name || '' });
      setView('form');
    } catch (err) {
      setError(err.message || 'Failed to load E-Library category.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (row, event) => {
    ...
    try {
      const result = await updateElibraryCategoryStatus(row.id, row.status);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = editId
        ? await updateElibraryCategory(editId, form)
        : await createElibraryCategory(form);
      ...
      setError(err.message || 'Failed to save E-Library category.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      <MastersHeaderActions
        ...
        searchPlaceholder="Search E-Library categories"
        onAdd={openAdd}
      />
            ...
            {filteredRows.length === 0 && !loading ? (
              <tr>
                <td className={styles.emptyCell} colSpan={4}>
                  No E-Library category records found.
                </td>
              </tr>
            ) : null}
