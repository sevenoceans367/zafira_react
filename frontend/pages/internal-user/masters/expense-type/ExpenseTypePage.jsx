...
} from '../../../../services/expenseTypes.js';
import MastersHeaderActions from '../MastersHeaderActions.jsx';
import { filterMasterRows } from '../filterMasterRows.js';
import styles from './ExpenseTypePage.module.css';

const FLASH_MESSAGES = {
  0: { type: 'success', text: 'Congratulations! Expense Type added/updated successfully.' },
  1: { type: 'error', text: 'Sorry! there was an error while adding/updating Expense Type.' },
...
export default function ExpenseTypePage() {
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load expense type list.');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (id) => {
    ...
    setError('');
    try {
      const record = await fetchExpenseType(id);
      setEditId(id);
      setForm({
        name: record.name || '',
      ...
      setError(err.message || 'Failed to load expense type.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (row, event) => {
    ...
    try {
      const result = await updateExpenseTypeStatus(row.id, row.status);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = editId
        ? await updateExpenseType(editId, form)
  ...
  return (
    <div className={`zafira-page ${styles.page}`}>
      ...
      {loading ? <LoadingOverlay active label="Loading expense types…" /> : null}

      <h3 className={styles.title}>Expense Type List</h3>
